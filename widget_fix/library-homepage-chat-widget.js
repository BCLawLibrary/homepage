(function () {
  var options = {
    id: 11077,
    hash: "89869b0c11c3b20ddcf044f4db5e59e2",
    siteUrl: "https://lawanswers.bc.edu",
    iid: 729,
    onlinerules: [{ u: 0, d: [3538], c: "" }],
    width: 400,
    height: 340,
    color_backg: "#f9f9f9",
    color_head: "#286090",
    color_btn: "#FFFFFF",
    offline_url: "https://lawanswers.bc.edu/form.php?queue_id=2930",
  ...
  const removeCoverageFromRule = function (rule) {
      return {
        ...
        d: rule.d,
        c: rule.c,
        fallbackSeconds: rule.fallbackSeconds || 0,
      };
    },
    ...
    return (
      onlineRules.forEach((rule) => {
        if (!rule.c || 1 === rule.coverageType)
          return void finalRules.push(removeCoverageFromRule(rule));
        ...
        isCoverageInRange(hour, minute, ranges) &&
          finalRules.push(removeCoverageFromRule(rule));
      }),
      finalRules
    );
  };
  const buttonWidget = {
    ...
    saveAutoPopDeny: function () {
      const obj = { date: Math.floor(Date.now() / 1e3) };
      try {
    ...
    autoPopDenied: function () {
      try {
        ...
        obj = JSON.parse(obj);
        return (
          !(Math.floor(Date.now() / 1e3) - obj.date > 3600) ||
          (this.deleteAutoPopDeny(), !1)
        );
      } catch (e) {
    ...
    openChat: function () {
      "" === this.referer && (this.referer = window.location.href),
        "" === this.refererTitle &&
          window.document.title &&
          (this.refererTitle = window.document.title);
      let authId = 0;
      window.springSpace &&
        window.springSpace.la &&
        window.springSpace.la.Page &&
        window.springSpace.la.Page.auth_id &&
        (authId = window.springSpace.la.Page.auth_id);
      const widgetUrl = `${this.config.siteUrl}/chat/widget/${
        this.config.hash
      }?referer=${encodeURIComponent(
        this.referer
      )}&referer_title=${encodeURIComponent(
        this.refererTitle
      )}&auth_id=${authId}`;
      window.open(
        widgetUrl,
        "libchat",
        `toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=yes, copyhistory=no, width=${this.config.width}, height=${this.config.height}`
      );
    },
    ...
    showModal: function () {
      if (((this.autoload = !0), "undefined" == typeof A11yDialog)) return;
      const modalId = "libchat_modal_" + this.config.id;
      if (null !== document.getElementById(modalId)) return;
      (this.modal = document.createElement("div")),
        (this.modal.id = modalId),
        this.modal.setAttribute("aria-labelledby", "libchat_modal_title"),
        this.modal.setAttribute("aria-hidden", !0),
        (this.modal.style.zIndex = 10);
      const overlay = document.createElement("div");
      overlay.setAttribute("data-a11y-dialog-hide", "true");
      const dialog = document.createElement("div");
      dialog.setAttribute("role", "document"), dialog.classList.add("dialog");
      const dialog_title = document.createElement("h1");
      (dialog_title.id = "libchat_modal_title"),
        dialog_title.setAttribute("tabindex", 0),
        (dialog_title.innerHTML = this.config.autoload_head);
      const dialog_text = document.createElement("div");
      (dialog_text.id = "autoload_text"),
        (dialog_text.innerHTML = this.config.autoload_text);
      const dialog_btns = document.createElement("div");
      dialog_btns.id = "autoload_btn";
      const dialog_yes = document.createElement("button");
      dialog_yes.setAttribute("type", "button"),
        (dialog_yes.innerHTML = this.config.autoload_yes);
      let dialog_cal = null;
      this.config.cal_autoload &&
        ...
        "" !== this.config.cal_text &&
        ((dialog_cal = document.createElement("button")),
        dialog_cal.setAttribute("type", "button"),
        (dialog_cal.innerHTML = this.config.cal_text));
      ...
      dialog_no.setAttribute("type", "button"),
        ...
        dialog_yes.addEventListener(
          "click",
          this.handleAutoPopAccept.bind(this)
        ),
        dialog_no.addEventListener("click", this.handleAutoPopDeny.bind(this)),
        null !== dialog_cal &&
          dialog_cal.addEventListener(
            "click",
            this.handleLibCalLink.bind(this)
          ),
    ...
    setTimer: function (isCoopOnline) {
      if (this.online) {
        ...
        this.config.autoload_time &&
          parseInt(this.config.autoload_time, 10) > 0 &&
          !this.autoPopDenied() &&
          (this.chatTimer = window.setTimeout(
            this.showModal.bind(this),
            1e3 * parseInt(this.config.autoload_time, 10)
          ));
    ...
    handleOnlineClick: function (ev) {
      ev.preventDefault(),
        ...
        this.online || "" === this.config.offline_url
          ? this.openChat()
          : (window.location.href = this.config.offline_url);
    },
    changeButtonStatus: function (online) {
      online !== this.online &&
        ((this.online = online),
        this.online
          ? null !== this.image
            ? (this.image.setAttribute("src", this.config.slidebutton_url),
              this.image.setAttribute("alt", this.config.slidebutton_text),
              this.button.setAttribute("href", "#"))
            : ((this.button.innerHTML = this.config.slidebutton_text),
              this.button.classList.add("libchat_online"),
              this.button.classList.remove("libchat_offline"))
          : null !== this.image
          ? (this.image.setAttribute("src", this.config.slidebutton_url_off),
            this.image.setAttribute("alt", this.config.slidebutton_text_off),
            "" !== this.config.offline_url &&
              this.button.setAttribute("href", this.config.offline_url))
          : ((this.button.innerHTML = this.config.slidebutton_text_off),
            this.button.classList.add("libchat_offline"),
            this.button.classList.remove("libchat_online")));
    },
    buildButton: function () {
      "" !== this.config.slidebutton_url_off &&
      "" !== this.config.slidebutton_url
        ? ((this.image = document.createElement("img")),
          this.image.classList.add("libchat_btn_img"),
          this.image.setAttribute("src", this.config.slidebutton_url_off),
          this.image.setAttribute("alt", this.config.slidebutton_text_off),
          (this.button = document.createElement("a")),
          this.button.setAttribute(
            "href",
            "" !== this.config.offline_url ? this.config.offline_url : "#"
          ),
        ...
        this.button.addEventListener(
          "click",
          this.handleOnlineClick.bind(this)
        ),
        this.chatContainer.appendChild(this.button);
    },
    statusSuccess: function (data) {
      let online = !1;
      (data.u || data.d || data.c) && (online = !0),
        this.changeButtonStatus(online);
      let isCoopOnline = !1;
      data.c && data.c.length > 0 && (isCoopOnline = !0),
        this.setTimer(isCoopOnline);
    },
    checkStatus: function () {
      const adjustedRules = adjustStatusRequestForCoverage(
          this.config.onlinerules
        ),
      ...
      (xhr.onload = () => {
        ...
        else this.changeButtonStatus(!1);
      }),
        (xhr.onerror = () => {
          this.changeButtonStatus(!1);
        }),
    ...
    start: function () {
      !0 !== this.loaded &&
        ...
        (this.chatContainer = document.querySelector(
          `#libchat_${this.config.hash}, #libchat_btn_widget, #libchat_d2o_widget`
        )),
        null !== this.chatContainer &&
          (this.insertWidgetCSS(),
          this.buildButton(),
          this.checkStatus(),
          document.addEventListener("visibilitychange", () => {
            "visible" === document.visibilityState && this.checkStatus();
          })));
    },
  };
  (buttonWidget.config = options),
    ...
    "complete" === document.readyState || "interactive" === document.readyState
      ...
      : (document.addEventListener(
          "DOMContentLoaded",
          buttonWidget.start.bind(buttonWidget),
          !1
        ),
