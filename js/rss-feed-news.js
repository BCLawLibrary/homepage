...
function feeds () {
  ...
  $.when(
	...
	var filteredPosts = posts.filter(item => !item.content.$t.includes('class="instagram-media"'));
    ...
    $.each(
      ...
      function(i, e){
		...
		if (i<=3) {
		  ...
		  formattedDate = postMonth+" "+postDay+", "+postYear;
		  ...
		  //Select one of 3 post types --short(display entire post)
		  //						   --long(display snippet and link to post)
		  // 						   --embed(show embedded content)
		  // 						   --title(show title only and link to post)
		...
		if (postType=='embed'){ //embeds don't need a thumbnail - assign the first default image just to be safe
			...
			} else { // Loop through however many default images we have specified.
		...
		var outerdiv =$('<div>')
			...
			.addClass('blogger-item-wrapper grid-item hover-drop-shadow');
		...
		/*
          ...
          switch(postType) {
            ...
            case 'title':
				...
				$(`<a class="clickable-link" href=${url}>Read more </a>`)
					.appendTo(textdiv);
            	break;
