...
function writeCovers(dataSource) {
    ...
    var link =$('<a>')
			.attr('href',coverLink)
			.addClass('new-book-cover hover-drop-shadow');
...
$(document).ready(function(){
  ...
  $.ajax({
    url: "https://sheets.googleapis.com/v4/spreadsheets/1Ym_DjK2JNfwv5HeTvF8wVtmOg-TXQZKwzEJg5gJunuA/values/A:N?key=AIzaSyD8Y28YJpVhE4XlVlOoA74Ws47YdPz5nGA"
  })
  .done(function( json ) {
    var myData = json['values']; //spreadsheet data lives in an array with the name values
       ...
       filteredData = myData.filter(val => val.featured == 'yes').slice(0, 6);
       console.log(filteredData);
       writeCovers(filteredData);
  });
});
